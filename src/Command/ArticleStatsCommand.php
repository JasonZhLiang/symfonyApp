...
class ArticleStatsCommand extends Command
{
    ...
    protected function execute(InputInterface $input, OutputInterface $output)
    {
        ...
        switch ($input->getOption('format')){
            case 'text':
                $io->listing($data); //here is output for print a list of things
                $rows =[];
                ...
                break;
            case 'json':
                $io->write(\GuzzleHttp\json_encode($data)); //output for print raw text
                $io->write(json_encode($data)); //identical with above, phpsotrm can suto complete this
                break;
            default:
                throw new \Exception('What kind of crazy fromat is that!?');
        }
